...
path = Path(__file__).resolve().parents[1] / "docs" / "thesis" / "05-phan-tich-class-diagram.docx"
errors = []
...
for paragraph in doc.paragraphs:
    if paragraph.style.name.startswith("Heading"):
        heading_count += 1
        heading_texts.append(paragraph.text.strip())
        if not paragraph.text.strip():
            errors.append("Empty heading")

required_headings = {
    "1. Mục đích và phạm vi",
    "2. Quan điểm mô hình hóa",
    "3. Phân rã lớp theo bounded context",
    "4. Mô hình lớp tổng quát",
    "5. Mô tả các lớp theo package",
    "6. Quan hệ và bội số giữa các lớp",
    "11. Danh mục Class Diagram cần trình bày",
}
